<template>
  <v-container v-if="pageReady" fluid class="pa-0 grey lighten-5">
    ...
    >
      {{ username }}/Project/{{ currentProject?.name }}/Issue
    </v-sheet>
...
export default {
  ...
  computed: {
    ...
    filteredItemsAllStates() {
      ...
      if (projectLabels && projectLabels.length > 0)
        result = result.filter((i) =>
          i.parsedLabels?.some((label) => projectLabels.includes(label))
        );
    ...
    filteredItems() {
      if (this.stateFilter && this.stateFilter !== "all")
        return this.filteredItemsAllStates.filter((i) => i.state === this.stateFilter);
      return this.filteredItemsAllStates;
    },
    openCount() {
      return this.filteredItemsAllStates.filter((i) => i.state === "opened").length;
    },
    closedCount() {
      return this.filteredItemsAllStates.filter((i) => i.state === "closed").length;
    },
    userFilterOptions() {
      return this.dbUsers || [];
    },
    ...
    selectedIssues() {
      ...
      return (this.filteredItems || []).filter((issue) =>
        selected.has(Number(issue.id))
      );
    },
    // label names ของ project ปัจจุบัน สำหรับ filter ออกจาก Add Labels > Project
    currentProjectLabelNames() {
      return this.activeLabelFilters || [];
    },
    // type 1 labels ที่ไม่ใช่ label ของ project ปัจจุบัน (ป้องกัน add ซ้ำ)
  ...
  async mounted() {
    try {
      ...
      // Restore filters from store or apply defaults
      const savedLabelFilter = this.$store.state.issue?.labelFilter;
      this.selectedStatusDisplay = savedLabelFilter || [];
      this.onStatusFilterChange(this.selectedStatusDisplay);
      if (this.$route.query.create === "1") this.showCreateDialog = true;
    } catch (error) {
      console.error("ไม่สามารถโหลดหน้า issue ได้", error);
    } finally {
      this.pageReady = true;
    }
  },
  ...
  methods: {
    ...
    async getUsers() {
      try {
        const res = await this.$store.dispatch("auth/getUsers", {
          limit: 500,
          page: 1,
        });
    ...
    onUserFilterChange(value) {
      this.$store.commit("issue/SET_USER_FILTER", value ? [value] : []);
    },
    onStatusFilterChange(value) {
      this.$store.commit("issue/SET_LABEL_FILTER", value);
    },
    ...
    async confirmDeleteIssue(issue) {
      ...
      try {
        ...
        if (!deleteResult.success)
          throw new Error(deleteResult.message || "Unknown error");
        ...
        this.$swal.fire("สำเร็จ", "ลบ Issue เรียบร้อยแล้ว", "success");
      } catch (error) {
        this.$swal.fire(
          "ผิดพลาด",
          `ลบ Issue ไม่สำเร็จ: ${error.message || "Unknown error"}`,
          "error"
        );
      }
    },
    async confirmRestoreIssue(issue) {
      if (!issue?.id) return;
      const result = await this.$swal.fire({
        title: "กู้คืน Issue นี้?",
      ...
      try {
        const restoreResult = await this.$store.dispatch(
          "issue/restoreIssue",
          issue.id
        );
        if (!restoreResult.success)
          throw new Error(restoreResult.message || "Unknown error");

        await this.getIssues();
        this.$swal.fire("สำเร็จ", "กู้คืน Issue เรียบร้อยแล้ว", "success");
      } catch (error) {
        this.$swal.fire(
          "ผิดพลาด",
          `กู้คืน Issue ไม่สำเร็จ: ${error.message || "Unknown error"}`,
          "error"
        );
      }
    },

    // ── Bulk Actions via events ──
    async applyBulkAddLabels(labelsToAdd) {
      ...
      try {
        ...
        for (const id of this.selectedIds) {
          const issue = (this.filteredItems || []).find(
    ...
    async confirmDeleteSelectedIssues() {
      ...
      const result = await this.$swal.fire({
        title: `ลบ Issue ที่เลือก ${count} รายการ?`,
        text: "ระบบจะเปลี่ยน state เป็น closed และทำ soft delete",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "ลบ",
      ...
      try {
        ...
        this.clearSelection();
        if (this.selectedIssue && ids.includes(Number(this.selectedIssue.id))) {
          this.selectedIssue = null;
          this.showIssueDetailDialog = false;
        }
        await this.getIssues();
        this.$swal.fire(
          "สำเร็จ",
          `ลบ Issue จำนวน ${count} รายการเรียบร้อยแล้ว`,
          "success"
        );
      } catch (error) {
        this.$swal.fire(
    ...
    async confirmRestoreSelectedIssues() {
      ...
      const count = this.selectedIds.length;
      const result = await this.$swal.fire({
        title: `กู้คืน Issue ที่เลือก ${count} รายการ?`,
        text: "ระบบจะเปลี่ยน state กลับเป็น opened",
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "กู้คืน",
      ...
      try {
        ...
        this.$swal.fire(
          "สำเร็จ",
          `กู้คืน Issue จำนวน ${count} รายการเรียบร้อยแล้ว`,
          "success"
        );
      } catch (error) {
        this.$swal.fire(
  ...
  head() {
    return { title: "Issues - GitLab Issue Tracker" };
  },
};
</script>
